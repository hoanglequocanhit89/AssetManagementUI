            ...
            key: 'action', actions: [
                    ...
                    render: (row) => {
                        ...
                        return (
                            <button disabled={isDisabled}>
                                <i className={`fa-solid fa-pen ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}></i>
                            </button>
                        );
                    },
                ...
                },
                {
                    render: (row) => {
                        ...
                        return (
                            <button disabled={isDisabled}>
                                <i className={`fa-regular fa-circle-xmark text-[var(--primary-color)] ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}></i>
                            </button>
                        );
                    },
...
const ManageAssignment = () => {
    ...
    useEffect(() => {
        const loginTest = async () => {
            if (IS_TEST_MODE) {
                setIsLoggedIn(true);
                return; // Bỏ qua login thật
            }
            try {
                await authApi.login("admindnu1", "admindnu1@01011990");
                setIsLoggedIn(true);
    ...
    }, [isLoggedIn, stateFilter, debouncedKeyword, sortFilter.sortBy, sortFilter.sortDir, pagingData.currentPage, assignedDateFilter]);
